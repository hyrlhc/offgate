...
export default function How() {
  return (
    <div className="page">
      ...
      <Section id="adimlar" kicker="Dört adım" title="Akış">
      ...
      <Section id="neden" kicker="Jürinin ilk sorusu" title="Neden zincir gerçekten gerekli">
        <div className="cols">
          <Card t="Merkezî sunucu bu işi yapamaz">
            İnternet yokken kimse sunucuya erişemez ve bakiyenin gerçekten kilitli olduğunu
            kimse doğrulayamaz. Burada kilit herkesçe görülebilir, kapı bunu çevrimdışı doğrular.
          </Card>
          ...
          <Card t="Hasılat gizlenemez">
            Kapı sayacı ve zincire düşen fişler bağımsız iki kaynaktır. Operatör yalnızca birini
            eksiltemez; fark denetim ekranında görünür.
          </Card>
        </div>
      </Section>

      <Section id="stellar" kicker="Ne, nerede" title="Stellar entegrasyonu">
        <div className="cols">
      ...
      <Section id="format" kicker="Tasarım kararı" title="Taşıma katmanı değiştirilebilir">
        <p className="prose">
      ...
      <footer className="foot">
        <div><b>OffGate</b> · Stellar Testnet · gerçek para hareketi yoktur</div>
...
function Flow({ n, t, d }: { n: string; t: string; d: string }) {
  return <li><span className="fn">{n}</span><div><h4>{t}</h4><p>{d}</p></div></li>;
}
